    ...
    <div className="bg-white dark:bg-slate-800 shadow-lg rounded-sm border border-slate-200 dark:border-slate-700 relative" dir="rtl">
      ...
      <div className="overflow-x-auto" dir="rtl">
        <table className="table-auto w-full dark:text-slate-300">
          ...
          <tbody className="text-sm font-medium divide-y divide-slate-200 dark:divide-slate-700">
            {filteredExpenses.map((expense) => (
              <tr key={expense.id} className="hover:bg-slate-100 dark:hover:bg-slate-800">
                <td className="p-2 text-slate-800 dark:text-slate-200">{expense.id}</td>
                ...
                <td className="p-2 text-slate-800 dark:text-slate-200">
                  {editingExpenseId === expense.id ? (
                    ...
                    <AddCommaToNumber number={expense.price} />
                  )}
                </td>
                <td className="p-2 text-slate-800 dark:text-slate-200">
                  {editingExpenseId === expense.id ? (
                    <Form.Control
                      type="datetime-local"
                      value={format(new Date(editedExpense.date_and_time), 'yyyy-MM-ddTHH:mm')}
                      onChange={(e) => handleEditChange(e, 'date_and_time')}
                      required
                    />
                  ) : (
                    format(new Date(expense.date_and_time), 'dd/MM/yyyy HH:mm')
                  )}
                </td>
                <td className="p-2 text-slate-800 dark:text-slate-200">
                  {editingExpenseId === expense.id ? (
                    <>
                      <Button onClick={saveChanges} disabled={!canSaveCreditCard} id="saveCreditCardChanges">שמור</Button>
                      <Button onClick={cancelEdit} className="ml-2">בטל</Button>
                    </>
                  ) : (
                    <>
                      <Button onClick={() => startEdit(expense)}>ערוך</Button>
                      <Button onClick={() => deleteExpense(token, expense.id, setExpenses)}>מחק</Button>
                    </>
                  )}
                </td>
              </tr>
