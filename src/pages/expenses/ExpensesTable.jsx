...
function ExpensesTable() {
  ...
  const startEdit = (expense) => {
    setEditingExpenseId(expense.id);
    setEditedExpense({ ...expense });
    if (expense.payment_method === 'כרטיס אשראי') {
  ...
  return (
    <div className="bg-white dark:bg-slate-800 shadow-lg rounded-sm border border-slate-200 dark:border-slate-700 relative" dir="rtl">
              ...
              filteredExpenses.map((expense, index) => (
                <tr key={expense.id}>
                  ...
                  <td className="p-2">
                    {editingExpenseId === expense.id ? (
                      <select
                        ...
                        onChange={(e) => handleEditChange(e, 'payment_method')}
                        onBlur={() => {
                          if (editedExpense.payment_method === 'כרטיס אשראי') {
                            setShowCreditCardModal(true);
                          }
                        }}
                      >
                  ...
                  <td className="p-2">
                    {editingExpenseId === expense.id ? (
                      <input
                        ...
                        id="date_and_time"
                        className="text-right"
                        value={editedExpense.date_and_time || ''}
                        onChange={(e) => handleEditChange(e, 'date_and_time')}
                      />
                    ) : (
                  ...
                  <td className="p-2">
                    {editingExpenseId === expense.id ? (
                      <>
                        <button onClick={saveChanges} disabled={editedExpense.payment_method === 'כרטיס אשראי' && !canSaveCreditCard}>שמור</button>
      ...
      <Modal show={showCreditCardModal} onHide={() => setShowCreditCardModal(false)}>
        <Modal.Header closeButton>
          <Modal.Title>בחירת כרטיס אשראי</Modal.Title>
        </Modal.Header>
        <Modal.Body>
          <Form>
            <Form.Group>
        ...
        <Modal.Footer>
          <Button variant="secondary" onClick={() => setShowCreditCardModal(false)}>
            סגור
          </Button>
        </Modal.Footer>
      </Modal>
